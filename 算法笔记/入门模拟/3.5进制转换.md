...
故通项是 n = n * N + m。(n:每次计算的结果，最终轮即转化后的结果。N:进制（就是几要转化成十进制就是几）。m:每一轮的余数（例子是2进制，所以余数只是1或者0，
"1010"，长度为4，即4个余数分别为1、0、1、0，怪不得16进制的余数要是0-9外加abcdef/ABCDEF（大小写根据题意来定），如果要是10、11、12、13、14、15表余数就分不
清是1、1（两次分别是1的余数）还是11了）)，只是这m的计算逻辑需要放进通项中计算前搞清楚，比如余数是a3cf的16进制，转换十进制为(((((0 * 16) + 10) * 16 + 3) * 16) + 12) * 16 + 15
